...
void joueur_opport_suppr(JoueurOpport& joueur) {
  // nothing to do.
}
...
void joueur_opport_tour(Jeu& jeu, JoueurOpport& joueur) {
  ...
  if(jeu_get_tour(jeu) == 0) {
    ...
    for(int firstround = 0 ; firstround < 4 ; firstround++) {
      ...
      joueur.wagons[temp.couleur].push_back(temp);
    }
  }
...
liste_liaison plus_court_chemin_opport(Jeu& jeu, const Objectif &obj, const int indice_joueur, const int ville_a, const int ville_b) { // Algo de Dijkstra (voir cours)
