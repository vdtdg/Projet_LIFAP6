...
void joueur_simple_tour(Jeu& jeu, JoueurSimple& joueur) {
  ...
  if(joueur.chemin_obj.empty() || joueur.chemin_obj.size() == 1) { // Si le chemin vers l'objectif est vide 
    if(joueur.obj.ville1 == 0 && joueur.obj.ville2 == 0) { // Si il n'y a pas d'objectif (ville1=ville2=0)
      ...
      // on cherche son plus court chemin
      ...
      if(joueur.chemin_obj.back() == -1) {
	if(DEBUG) { std::cout << "Le chemin vers ce nouvel objectif est impossible à parcourir, j'attends donc le prochain tour pour piocher un autre objectif." << std::endl; }
	joueur.chemin_obj.clear(); // on dégage le chemin vers l'objectif pour qu'au prochain tour, on puisse en prendre un autre
      }
    }
  }
  ...
  int ville2 = joueur.chemin_obj[joueur.chemin_obj.size() - 2]; // avant dernier elem
  ...
  while(prochain.proprietaire == joueur.indice) { // Si prochain est à moi, je passe à celle d'après
    ...
    if(joueur.chemin_obj.size() == 1) { // Si j'ai fini l'objectif, je termine le tour. 
  ...
  if(prochain.longueur - nombre_w_p <= 0) {
    ...
    for(unsigned int i = 0; i < joueur.chemin_obj.size(); i++) {
      if(DEBUG) { std::cout << " " << joueur.chemin_obj[i]; }
    }
